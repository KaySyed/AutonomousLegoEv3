...
def detect_face(img):
    ...
    faces = face_cascade.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=5, flags=0, minSize=(30,30))
...
def prepare_training_data(data_folder_path):
    ...
    for dir_name in dirs:
        ...
        for image_name in subject_images_names:
            ...
            #we will ignore faces0 that are not detected
            if face is not None:
...
#load test images
test_img1 = cv2.imread("test_images/t1.jpg")
...
#display both images
cv2.imshow(subjects[1], predicted_img1) #cv2.resize(predicted_img1, (400, 500)))
cv2.waitKey(0)
cv2.destroyAllWindows()
cv2.waitKey(1)
cv2.destroyAllWindows()
